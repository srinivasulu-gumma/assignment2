...
app.get("/user/tweets/feed", authenticateToken, async (request, response) => {
  ...
  console.log(name);
  const getTweetsFeedQuery = `SELECT username, tweet, date_time AS dateTime FROM 
...
app.get("/user/followers", authenticateToken, async (request, response) => {
  ...
  const { user_id, name, username, gender } = payload;
  console.log(name);
  const userFollowsQuery = `SELECT name FROM user INNER JOIN follower ON user.user_id = follower.following_user_id
...
app.get("/user/followers", authenticateToken, async (request, response) => {
  ...
  const { user_id, name, username, gender } = payload;
  console.log(name);
  const userFollowsQuery = `SELECT name FROM user INNER JOIN follower ON user.user_id = follower.follower_user_id
                                WHERE 
...
app.get(
  ...
  async (request, response) => {
    ...
    const { payload } = request;
    const { user_id, name, username, gender } = payload;
    const getLikedUsersQuery = `SELECT * FROM follower INNER JOIN tweet.user_id = follower.following_user_id INNER JOIN like ON like.tweet_id = tweet.tweet_id
...
app.delete("/tweets/:tweetId", authenticateToken, async (request, response) => {
  ...
  if (tweetUser.length !== 0) {
    const deleteTweetQuery = `DELETE FORM tweet WHERE tweet.user_id = '${user_id}' AND tweet.tweet_id = '${tweetId}';`;
